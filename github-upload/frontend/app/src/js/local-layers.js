...
async function importMBTilesWeb(file, name) {
  // Check file size — warn if over 500MB on web
  if (file.size > 500 * 1024 * 1024) {
    throw new Error(`File is ${(file.size / (1024*1024)).toFixed(0)}MB — too large for browser. Use the app (APK) for files over 500MB.`);
  }
...
function createNativeTileLayer(id, name) {
  ...
  const LayerClass = L.GridLayer.extend({
    createTile: function(coords, done) {
      ...
      plugin.getTile({ id, z: coords.z, x: coords.x, y: coords.y }).then(result => {
        if (result.data) {
