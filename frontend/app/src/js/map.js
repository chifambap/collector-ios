...
export function initMap() {
  ...
  state.map.on('click', e => {
    ...
    if (sidebar.classList.contains('open') && window.innerWidth <= 768) {
      ...
      btn.textContent = '📋 ' + (window.t ? window.t('map.showpanel','Show Panel') : 'Show Panel');
    }
...
// Toggle two-finger map rotation on/off
export function toggleRotation() {
  if (!state.map) return;
  const btn = document.getElementById('map-rotate-btn');
  const locked = localStorage.getItem('gc_rotate_locked') === '1';
  if (locked) {
    state.map.touchRotate.enable();
    if (state.map.shiftKeyRotate) state.map.shiftKeyRotate.rotate = true;
    localStorage.setItem('gc_rotate_locked', '0');
    if (btn) btn.textContent = '🔓';
    if (btn) btn.title = 'Rotation on — tap to lock';
  } else {
    state.map.touchRotate.disable();
    if (state.map.shiftKeyRotate) state.map.shiftKeyRotate.rotate = false;
    state.map.setBearing(0);
    localStorage.setItem('gc_rotate_locked', '1');
    if (btn) btn.textContent = '🔒';
    if (btn) btn.title = 'Rotation locked — tap to unlock';
  }
}
window.toggleRotation = toggleRotation;

// Apply saved rotation lock state on startup
export function applyRotationLock() {
  if (localStorage.getItem('gc_rotate_locked') === '1') {
    if (state.map) {
      state.map.touchRotate.disable();
      if (state.map.shiftKeyRotate) state.map.shiftKeyRotate.rotate = false;
    }
    const btn = document.getElementById('map-rotate-btn');
    if (btn) { btn.textContent = '🔒'; btn.title = 'Rotation locked — tap to unlock'; }
  }
}
...
export function startDraw(type) {
  if (!state.drawControl || !state.map) return;
  if (type === 'polygon') new L.Draw.Polygon(state.map, state.drawControl.options.draw.polygon).enable();
  if (type === 'marker')  new L.Draw.Marker(state.map,  state.drawControl.options.draw.marker).enable();
}
