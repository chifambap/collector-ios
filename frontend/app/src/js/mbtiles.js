    ...
    listEl.innerHTML = items.map(mbt => `
      <div style="display:flex;align-items:center;gap:.5rem;padding:.4rem 0;border-bottom:1px solid var(--border)">
        ...
        <button class="btn btn-ghost" style="padding:.3rem .6rem;font-size:.7rem"
                onclick="toggleMBTilesLayer(${mbt.id}, '${attr(mbt.name)}', '${attr(mbt.bounds || '')}', ${mbt.max_zoom || 18})"
                id="mbt-btn-${mbt.id}">
          Load
        </button>
        ${!isAdmin ? `
...
export function toggleMBTilesLayer(id, name, boundsStr, maxNativeZoom) {
  ...
  const layer = L.tileLayer(authUrl, {
    minZoom: 0, maxZoom: 24, maxNativeZoom: maxNativeZoom || 18, tileSize: 256,
    attribution: `📂 ${name}`,
...
export function addMBTilesLayerToMap(mbt) {
  toggleMBTilesLayer(mbt.id, mbt.name, mbt.bounds || '', mbt.max_zoom || 18);
}
