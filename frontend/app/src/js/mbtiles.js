...
export async function uploadMBTiles() {
  ...
  try {
    const data = await mbtilesAPI.upload(
      file,
      nameInput.value || file.name.replace('.mbtiles', ''),
      '',
      (pct, phase) => {
      ...
      <div style="display:flex;align-items:center;gap:.5rem;padding:.4rem 0;border-bottom:1px solid var(--border)">
        <div style="flex:1;min-width:0">
          ...
          <div style="font-size:.67rem;color:var(--txt2);font-family:var(--mono)">
            z${esc(mbt.min_zoom)}–${esc(mbt.max_zoom)} · ${esc(mbt.file_size_mb)} MB
          </div>
        </div>
        <button class="btn btn-ghost" style="padding:.3rem .6rem;font-size:.7rem"
                onclick="toggleMBTilesLayer(${mbt.id}, '${attr(mbt.name)}', '${attr(mbt.bounds || '')}', ${Number(mbt.max_zoom) || 0}, ${Number(mbt.min_zoom) || 0})"
                id="mbt-btn-${mbt.id}">
          Load
        </button>
        ${!isAdmin ? `
        <button class="btn btn-ghost" style="padding:.3rem .6rem;font-size:.7rem"
                onclick="saveForOffline(${mbt.id}, '${attr(mbt.name)}')"
...
/**
 * Zoom options for a server MBTiles layer.
 *
 * maxNativeZoom marks the deepest zoom the file actually contains; past it
 * Leaflet upscales the last real tile instead of requesting tiles that do not
 * exist and drawing blank. A missing or 0 max_zoom must not be read as 18 —
 * that is a promise of detail the file cannot keep.
 */
function zoomOptions(minNativeZoom, maxNativeZoom) {
  const max = Number(maxNativeZoom);
  const min = Number(minNativeZoom);
  const opts = { minZoom: 0, maxZoom: 24, tileSize: 256 };
  // Outside 1..24 is not a real zoom level: treat as unknown rather than
  // clamping to 24, which would leave no headroom above the layer's maxZoom.
  opts.maxNativeZoom = (Number.isFinite(max) && max > 0 && max <= 24) ? Math.round(max) : 18;
  if (Number.isFinite(min) && min > 0) opts.minNativeZoom = Math.min(Math.round(min), opts.maxNativeZoom);
  return opts;
}

export function toggleMBTilesLayer(id, name, boundsStr, maxNativeZoom, minNativeZoom) {
  ...
  const layer = L.tileLayer(authUrl, {
    ...zoomOptions(minNativeZoom, maxNativeZoom),
    attribution: `📂 ${name}`,
...
export function addMBTilesLayerToMap(mbt) {
  toggleMBTilesLayer(mbt.id, mbt.name, mbt.bounds || '', mbt.max_zoom, mbt.min_zoom);
}
