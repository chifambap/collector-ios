...
// Escape user/server-sourced strings before inserting into innerHTML
...
// Escape a string for use inside a JS string literal inside an onclick="..." attribute
...
export function persist() {
  try {
    const toSave = state.fields.map(f => ({
      type: f.type,
      geometry: f.geometry,
      // _clientUUID is the server's ONLY way to recognise an entry it already
      // holds. It lives on the feature, not in properties, so rebuilding the
      // object here used to drop it: after any reload every entry looked new,
      // got a fresh uuid, and "Send to Server" re-uploaded the whole dataset
      // as duplicates. Keep it.
      _clientUUID: f._clientUUID,
      properties: {
        ...f.properties,
