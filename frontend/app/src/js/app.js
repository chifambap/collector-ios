...
async function checkForUpdate() {
  try {
    const res = await fetch(`https://geos.zingsageocrops.com/api/app/version/`);
    const data = await res.json();
    if (data.version_code > APP_VERSION_CODE) {
      const bar = document.getElementById('update-bar');
      document.getElementById('update-version').textContent = 'v' + data.version_name;
      document.getElementById('update-link').href = `https://geos.zingsageocrops.com${data.download_url}`;
