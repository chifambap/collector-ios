...
import { initMap, updateDrawControl,
         ...
         applyRotationLock }                     from './map.js';
import { updateStats, renderEntries,
         buildLegend, showPanel, setMode,
         fillAreaFromGeometry }                  from './entries.js';
...
setDrawCreatedCallback(() => {
  fillAreaFromGeometry();               // auto-fill Area (Ha) for polygons
...
// ── Status bar: edge-to-edge on Capacitor (Android + iOS) ───────────────────
async function initStatusBar() {
  ...
  } catch (_) { /* plugin not available in browser */ }
}
...
async function checkForUpdate() {
  try {
    ...
    if (data.version_code > APP_VERSION_CODE) {
      ...
      document.getElementById('update-link').href = `${API_BASE.replace('/api', '')}${data.download_url}`;
...
function setConnStatus(online) {
  ...
  document.getElementById('conn-lbl').textContent = window.t ? window.t(online?'nav.online':'nav.offline') : (online ? 'Online' : 'Offline');
  document.getElementById('offline-bar').classList.toggle('show', !online);
}

// ── Bootstrap ─────────────────────────────────────────────────────────────────
// ── Header collapse (Capacitor mobile only) ──────────────────────────────────
function toggleHeader() {
  const hdr = document.querySelector('header');
  const btn = document.getElementById('header-toggle-btn');
  const hidden = hdr.classList.toggle('header-hidden');
...
function initHeaderCollapse() {
  ...
  document.body.classList.add('cap-mobile');
}
