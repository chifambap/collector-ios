...
const _SURVEYS_CACHE = 'gc_surveys_cache';
let _surveys = [];
...
export async function loadSurveys() {
  ...
  try {
    ...
    localStorage.setItem(_SURVEYS_CACHE, JSON.stringify(_surveys));
    populateSurveyDropdown();
    updateSurveysBtnVisibility();
  } catch (e) {
    // Network unavailable — restore from cache so offline users can still select a survey
    const cached = localStorage.getItem(_SURVEYS_CACHE);
    if (cached) {
      try {
        _surveys = JSON.parse(cached);
        populateSurveyDropdown();
        updateSurveysBtnVisibility();
      } catch (_) {}
    }
    console.warn('Surveys loaded from cache (offline):', e.message);
  }
}
...
function renderSurveyList() {
  ...
  el.innerHTML = _surveys.map(s => `
    <div class="survey-row">
      ...
      <span class="survey-count">${esc(s.entry_count || 0)} entries</span>
