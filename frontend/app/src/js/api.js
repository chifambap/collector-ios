...
// Offline-only: protects cached credential against opportunistic device access
// (attacker has physical device + unlocked localStorage).  NOT equivalent to
// server-side PBKDF2 (Django uses 720 000 iterations).  100 000 rounds of
// iterated SHA-256 is a deliberate trade-off: mobile WebView pure-JS execution
// is ~5x slower than native, so 100k rounds ≈ 300-500 ms on a mid-range phone
// — acceptable for a login action, while raising the brute-force cost ~10x over
// the original 10k.  OWASP's 600k+ guidance targets server-side PBKDF2 with
// native C implementations; applying it to a pure-JS hot loop would freeze the
// UI for several seconds.
const OFFLINE_HASH_ITERATIONS = 100_000;

function hashPassword(password, saltHex) {
  let h = sha256(saltHex + password);
  for (let i = 1; i < OFFLINE_HASH_ITERATIONS; i++) h = sha256(saltHex + h);
  return h;
}
