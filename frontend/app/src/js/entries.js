...
import { cap, toast, persist, f2b, showHint, generateUUID } from './utils.js';
...
// Auto-fill the Area (Ha) field from the drawn polygon's geodesic area.
// Points/markers have no area, so they are skipped. Rectangles count (polygons).
export function fillAreaFromGeometry() {
  const layer = state.pendingLayer;
  const input = document.getElementById('area-ha');
  if (!layer || !input) return;
  if (typeof layer.getLatLngs !== 'function') return;   // marker/point → no area
  try {
    let ring = layer.getLatLngs();
    while (Array.isArray(ring) && ring.length && Array.isArray(ring[0])) ring = ring[0]; // outer ring
    if (!ring || ring.length < 3 || !L.GeometryUtil || !L.GeometryUtil.geodesicArea) return;
    const ha = L.GeometryUtil.geodesicArea(ring) / 10000;   // m² → hectares
    if (!isFinite(ha) || ha <= 0) return;
    input.value = ha.toFixed(2);
    input.dispatchEvent(new Event('input', { bubbles: true }));  // float the floating label
    toast(`📐 Area calculated: ${ha.toFixed(2)} ha`, 'inf');
  } catch (e) { /* leave area untouched on any failure */ }
}
...
export function showPanel(name, opts = {}) {
  ...
  if (opts.openSidebar) {
    ...
    if (sidebar && btn && !sidebar.classList.contains('open')) {
      ...
      btn.textContent = '🗺️ ' + (window.t ? window.t('map.showmap','Show Map') : 'Show Map');
    }
  }
}
...
function clearAttributeFields() {
  ...
  if (pl) pl.textContent = window.t ? window.t('collect.photos','Attach field photos (optional)') : 'Attach field photos (optional)';
}
...
export function startMultiCropWizard() {
  ...
  // Keep any geometry the user already drew (and any attributes/area they typed)
  // so "draw polygon → fill form → Start Capture" carries straight into crop 1
  // instead of wiping the polygon.
...
function showCropWizardStep(idx) {
  ...
  const label    = `${emoji} ${window.gcCrop ? window.gcCrop(cropType) : cap(cropType)}`.trim();
...
async function buildDraft(cropType) {
  ...
  return {
    ...
    feat: {
      ...
      // Identity is fixed when the observation is captured, not when it is
      // sent, so a resend is always recognised as the same entry.
...
export function renderEntries() {
  ...
  el.innerHTML = list.map(f => {
    ...
    const tr = (g, k) => (window.gcEnum ? window.gcEnum(g, k) : String(k).replace(/_/g, ' '));
    const cropName = window.gcCrop ? window.gcCrop(p.cropType) : cap(p.cropType || 'Unknown');
    const vChip = p.validation
      ? `<span class="chip ${p.validation.status === 'correct' ? 'c-green' : p.validation.status === 'incorrect' ? 'c-red' : 'c-amber'}">${tr('vstatus', p.validation.status)}</span>`
      : `<span class="chip c-grey">${window.t ? window.t('entries.unvalidated', 'Unvalidated') : 'Unvalidated'}</span>`;
    const gChip = p.growthStage ? `<span class="chip c-teal">${tr('growth', p.growthStage)}</span>` : '';
    const cChip = p.cropCondition ? `<span class="chip c-amber">${tr('condition', p.cropCondition)}</span>` : '';
    const iChip = p.irrigation ? `<span class="chip c-blue">${tr('irrigation', p.irrigation)}</span>` : '';
    ...
    return `<div class="entry" onclick="zoomTo(${ri})">
      ...
      <div class="entry-body">
        <div class="entry-name">${em} ${cropName}</div>
...
// Re-render dynamic lists when the UI language changes.
window.addEventListener('gc:langchange', () => {
  try { renderEntries(); updateStats(); buildLegend(); } catch (e) {}
});
...
function toggleSidebar() {
  ...
  btn.textContent = isOpen ? ('🗺️ ' + (window.t?window.t('map.showmap','Show Map'):'Show Map')) : ('📋 ' + (window.t?window.t('map.showpanel','Show Panel'):'Show Panel'));
}
