...
export function selectForValidation(idx) {
  ...
  const cropName = window.gcCrop ? window.gcCrop(f.properties.cropType) : cap(f.properties.cropType);
  document.getElementById('val-feat-name').textContent =
    `${CROP_EMOJI[f.properties.cropType] || '🌿'} ${cropName} — ${new Date(f.properties.timestamp).toLocaleDateString()}`;
  rebuildLayers();
...
export async function submitValidation() {
  ...
  if (sidebar.classList.contains('open')) {
    ...
    document.getElementById('sidebar-toggle').textContent = '📋 ' + (window.t ? window.t('map.showpanel','Show Panel') : 'Show Panel');
  }
}
