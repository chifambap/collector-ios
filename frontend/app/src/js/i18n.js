 ...
 *     before production. Entries still equal to the English text are
...
(function (w) {
  ...
  var S = {
    ...
    'signin.orportal':   { en: 'Or go directly to a portal', sn: 'Kana enda kuportal', nd: 'Kumbe uye ngqo kuphothali' },
    'signin.fieldapp':   { en: 'Field Collection App', sn: 'App yeKutora Zvirimwa', nd: 'I-App yokuQoqa yeSimu' },
    ...
    'upload.armnote':    { en: 'Slide to unlock, then confirm. Re-locks by itself.',
                           sn: 'Tsvedzera kuti uzarure, wozosimbisa. Inozvivhara yoga.',
                           nd: 'Tshelela ukuvula, ubusuqinisa. Iyazivala yodwa.' },
    'upload.armhint':    { en: '🔒 Slide to unlock →', sn: '🔒 Tsvedzera kuzarura →', nd: '🔒 Tshelela ukuvula →' },
    'upload.armed':      { en: '🔓 Unlocked', sn: '🔓 Yazaruka', nd: '🔓 Ivuliwe' },
    'upload.armfirst':   { en: 'Slide to unlock first', sn: 'Tanga watsvedzera kuzarura', nd: 'Qala ngokutshelela ukuvula' },
    'upload.armaria':    { en: 'Slide to unlock Clear All Data',
                           sn: 'Tsvedzera kuzarura Bvisa Data Rese',
                           nd: 'Tshelela ukuvula Sula Yonke Idatha' },
    ...
    'crop.blueberry':    { en: 'Blueberry', sn: 'Blueberry', nd: 'Iblueberry' },
    'crop.mango':        { en: 'Mango', sn: 'Mango', nd: 'Umango' },
    'crop.guava':        { en: 'Guava', sn: 'Gwavha', nd: 'Ugwava' },
    'crop.coffee':       { en: 'Coffee', sn: 'Kofi', nd: 'Ikofi' },
    'crop.lemon':        { en: 'Lemon', sn: 'Lemoni', nd: 'Ulemoni' },
    'crop.orange':       { en: 'Orange', sn: 'Ranjisi', nd: 'Iwolintshi' },
    'crop.other_citrus': { en: 'Other Citrus', sn: 'Mimwe michero yeranjisi', nd: 'Amanye amahlobo ewolintshi' },
    'crop.pasture_fodder':{ en: 'Pasture/Fodder', sn: 'Mafuro/Chikafu chezvipfuyo', nd: 'Idlelo/Ukudla kwezifuyo' },
    'crop.fallow':       { en: 'Fallow', sn: 'Munda usina kudyarwa', nd: 'Insimu engalinywanga' },
    'crop.other_legumes':{ en: 'Other legumes', sn: 'Mimwe mhodzi', nd: 'Amanye amabhinzi' },
    'crop.vegetables':   { en: 'Vegetables', sn: 'Miriwo', nd: 'Imibhida' },
    'crop.intercrop':    { en: 'Intercrop', sn: 'Kusanganisa zvirimwa', nd: 'Ukuxuba izilimo' },
  ...
  //
  // Every lookup passes the markup's existing text as the fallback. An unknown
  // key then leaves the authored English in place instead of replacing it with
  // the raw key — which is what users saw ("upload.armnote") whenever the HTML
  // shipped ahead of this file, or a translation was simply forgotten.
  function applyI18n(root) {
    ...
    for (i = 0; i < list.length; i++) {
      ...
      el.textContent = (pfx ? pfx : '') + t(el.getAttribute('data-i18n'), el.textContent);
    }

    list = root.querySelectorAll('[data-i18n-html]');
    for (i = 0; i < list.length; i++) { el = list[i]; el.innerHTML = t(el.getAttribute('data-i18n-html'), el.innerHTML); }

    list = root.querySelectorAll('[data-i18n-ph]');
    for (i = 0; i < list.length; i++) { el = list[i]; el.setAttribute('placeholder', t(el.getAttribute('data-i18n-ph'), el.getAttribute('placeholder'))); }

    list = root.querySelectorAll('[data-i18n-title]');
    for (i = 0; i < list.length; i++) { el = list[i]; el.setAttribute('title', t(el.getAttribute('data-i18n-title'), el.getAttribute('title'))); }

    list = root.querySelectorAll('[data-i18n-aria]');
    for (i = 0; i < list.length; i++) { el = list[i]; el.setAttribute('aria-label', t(el.getAttribute('data-i18n-aria'), el.getAttribute('aria-label'))); }
