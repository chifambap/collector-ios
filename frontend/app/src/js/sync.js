...
/** Safety stop so a bad cursor can't spin forever against the server. */
...
export async function pullFromServer(onProgress) {
  const meta = await idbGet(META_STORE, 'last_pull_ts');
  let since = meta?.value || null;

  // The server caps rows per response. Keep requesting until it reports no more,
  // advancing the cursor to the last row received — advancing to server_ts
  // instead would silently skip everything past the first page.
  const all = [];
  for (let page = 0; page < MAX_PULL_PAGES; page++) {
    const result = await syncAPI.pull(since);
    const entries = result.entries || [];
    // Appended one at a time, not push(...entries): a page can hold
    // SYNC_PULL_MAX_ROWS rows, which is derived from server RAM and can exceed
    // the ~125k argument limit that a spread would hit.
    for (const entry of entries) all.push(entry);

    // Persist after every page so an interrupted sync resumes instead of restarting.
    since = result.next_since || result.server_ts;
    await idbPut(META_STORE, { key: 'last_pull_ts', value: since });

    if (!result.has_more) return all;
    if (onProgress) onProgress(`Pulling… ${all.length} entries`);
  }

  console.warn(`Pull stopped at ${MAX_PULL_PAGES} pages; more data remains.`);
  return all;
}
...
export async function fullSync(onProgress) {
  ...
  try {
    ...
    const pulled = await pullFromServer(log);
