...
async function _parseOverlayResponse(response, ext) {
  if (ext === 'gpkg') {
    return gpkgToGeoJSON(await response.arrayBuffer());
  }
...
export async function importLocalOverlay(file, name) {
  ...
  let geojson;
  if (ext === 'gpkg') {
    geojson = await gpkgToGeoJSON(await file.arrayBuffer());
  } else {
...
export async function toggleLocalOverlayLayer(id, name, boundsStr) {
  ...
  try {
    ...
    if (isCapacitor) {
      // Read from native disk via Capacitor's local file interceptor — no bridge overhead
      ...
      geojson = await _parseOverlayResponse(response, meta.ext);
    } else {
      ...
      return `
        <div style="display:flex;align-items:center;gap:.5rem;padding:.4rem 0;border-bottom:1px solid var(--border)">
          ...
          <button class="btn btn-ghost" style="padding:.3rem .6rem;font-size:.7rem${loaded ? ';color:var(--earth)' : ''}"
                  onclick="toggleLocalMBTilesLayer('${attr(mbt.id)}', '${attr(mbt.name)}', '${attr(mbt.bounds)}')"
                  id="lmbt-btn-${mbt.id}">
            ${loaded ? 'Loaded' : 'Load'}
          </button>
...
async function importOverlayNative(name) {
  ...
  const meta = {
    ...
    fileFormat: result.ext === 'kml' ? 'kml' : result.ext === 'gpkg' ? 'gpkg' : 'geojson',
