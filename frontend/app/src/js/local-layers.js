...
async function importMBTilesNative(file, name) {
  ...
  _mergeNativeMBTilesMeta(record);
  return record;
}
...
/** Native (Capacitor) MBTiles zoom metadata keyed by id (`mbt_*`). Not persisted; filled from plugin.list() and import. */
const _nativeMBTilesMeta = {};

function _mergeNativeMBTilesMeta(entry) {
  if (!entry?.id) return;
  const minZ = entry.minZoom;
  const maxZ = entry.maxZoom;
  _nativeMBTilesMeta[entry.id] = {
    minZoom: typeof minZ === 'number' ? minZ : parseInt(minZ ?? '0', 10),
    maxZoom: typeof maxZ === 'number' ? maxZ : parseInt(maxZ ?? '18', 10),
    bounds: entry.bounds || '',
  };
}
...
export async function listLocalMBTiles() {
  if (isCapacitor) {
    ...
    for (const r of items) _mergeNativeMBTilesMeta(r);
    return items.map(r => ({
      id: r.id, name: r.name,
...
function createNativeTileLayer(id, name) {
  const plugin = getNativePlugin();
  const meta = _nativeMBTilesMeta[id] || _webMBTilesMeta[id];
  const maxNative = meta?.maxZoom ?? 18;
  ...
  return new LayerClass({
    minZoom: 0, maxZoom: 24, maxNativeZoom: maxNative, tileSize: 256,
