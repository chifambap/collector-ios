...
function showLoginGate() {
  ...
  const sel = document.getElementById('gate-server-domain');
  if (sel) sel.value = localStorage.getItem('gc_server_domain') || 'geos.zingsageocrops.com';
}
...
export async function handlePinSetupSave(e) {
  e.preventDefault();
...
export async function handleAccountEnableAppLock(e) {
  e.preventDefault();
...
window.addEventListener('gc:langchange', () => { try { updateAuthUI(); } catch (e) {} });
...
export function updateAuthUI() {
  ...
  if (state.currentUser) {
    ...
    btn.textContent = '👤 ' + (window.t ? window.t('auth.login', 'Login') : 'Login');
...
export function openAuthModal() {
  ...
  if (state.currentUser) {
    ...
    document.getElementById('user-role').textContent =
      window.t ? window.t('role.' + (state.currentUser.role || 'collector'), state.currentUser.role || 'collector')
               : (state.currentUser.role || 'collector');
    refreshAppLockSection();
  } else {
    switchAuthTab('login');
  }
...
export async function handleLogin(e) {
  ...
  try {
    ...
    await _handleLoginError(err, username, password, showAuthError, () => {
      closeAuthModal();
      hideLoginGate();
      e.target.reset();
    });
...
export async function handleGateLogin(e) {
  ...
  const sel = document.getElementById('gate-server-domain');
  if (sel) {
    const prev = localStorage.getItem('gc_server_domain') || 'geos.zingsageocrops.com';
    if (sel.value !== prev) {
      localStorage.setItem('gc_server_domain', sel.value);
  ...
  try {
    ...
    await _handleLoginError(err, username, password, showErr, () => {
      hideLoginGate();
      e.target.reset();
    });
  } finally {
...
async function _handleLoginError(err, username, password, showErr, onSuccess) {
  ...
  if (networkFail) {
    const result = await verifyOfflineCredentials(username, password);
    if (result.ok) {
